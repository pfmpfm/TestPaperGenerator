...
┌─────────────────────────────────────────────────────────┐
│                    Vue 3 前端                            │
│  CreateView | ProgressView | PreviewView | Upload       │
└──────────────────────────┬──────────────────────────────┘
                           │ HTTP/WebSocket
┌──────────────────────────▼──────────────────────────────┐
│              FastAPI Backend (2 副本)                    │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────┐ │
...
用户需求输入
    ↓
解析需求 (parse_requirement)
    ↓
并发生成题目 (generate_questions)
    ├─ RAG: 检索历史相似题（避免重复）
    ├─ RAG: 检索教材知识点（提供上下文）
    └─ LLM: 批量生成题目（超额 50%）
    ↓
质量审核 (review_questions) - AutoGen 3-Agent 群聊
    ├─ 内容审核员：检查题目规范性
    ├─ 答案校验员：验证答案正确性
    └─ 决策员：综合评分决策
    ↓
去重入库 (deduplicate_questions)
    └─ 向量相似度 > 0.85 视为重复
    ↓
补偿生成 (compensate)
    └─ 数量不足时自动补齐（最多 3 轮）
